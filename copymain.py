...
def balancing(train_paths, train_labels, valid_paths, valid_labels):
    ...
    names = ["fake train samples", "real train samples", "fake val samples", "real val samples"]
    scores = [y.count(1), y.count(0), val_y.count(1), val_y.count(0)]
    postitions = [0, 1, 2, 3]

    import matplotlib.pyplot as plt
    plt.bar(postitions, scores, width=0.5)
    plt.xticks(postitions, names)
...
def define_model_lstm():
    ...
    learning_model.add(
        LSTM(units=2048, input_shape=(12, 2048), dropout=0.5))  # input_shape = sequence length, feature vector length
...
def get_feature_vector():
    ...
    for vector in tqdm(list_folder_features_train):
        ...
        try:
            ...
            print("deleted")
            os.rmdir('./data/deepfake_features/train/' + vector + '/')
        if label == 'REAL':
    ...
    for vector in tqdm(list_folder_features_validation):
        ...
        try:
            ...
            print("passed")
            os.remove('./data/deepfake_features/validation/' + vector + '/' + name + '.npy')
            os.rmdir('./data/deepfake_features/validation/' + vector + '/')
        except FileNotFoundError:
            print("deleted")
            os.rmdir('./data/deepfake_features/validation/' + vector + '/')
...
videos_path, videos_label, valid_videos_path, valid_videos_labels = preprocessing('D:\\Deep_Fake\\dfdc_train_all\\')
...
clean_valid_X, clean_valid_y = [], []
for i in range(len(val_X)):
    temp = val_X[i]
    if temp.shape[0] > 12:
        temp = temp[0:12]
    clean_valid_X.append(temp)
    clean_valid_y.append(val_y[i])

from keras.utils import to_categorical
train_labels = to_categorical(clean_y)
validation_labels = to_categorical(clean_valid_y)
print(clean_y)
#
# # roc curve and auc
# import sklearn
...
# list_models, list_losses = train(lstm, clean_X, train_labels, clean_valid_X, validation_labels)
#%%

# 6) predict validation set
#list_models[0] = final_model
best_model_pred = final_model.predict([clean_valid_X])
best_model = final_model
model_pred = prediction_pipline(clean_valid_X, [best_model])
#%%
final_preds =[]
final_labels = []

for i in range(len(best_model_pred)):
    tmp = best_model_pred[i]
    final_preds.append(tmp[1])
    final_labels.append(int(validation_labels[i][1]))


#%%
# 7) Check performance of models
random_pred = np.random.random(len(clean_valid_X))
print('random loss: ' + str(log_loss(validation_labels, random_pred.clip(0.35, 0.65))))
allone_pred = np.array([1 for _ in range(len(clean_valid_X))])
print('1 loss: ' + str(log_loss(validation_labels, allone_pred)))
allzero_pred = np.array([0 for _ in range(len(clean_valid_X))])
print('0 loss: ' + str(log_loss(validation_labels, allzero_pred)))
allpoint5_pred = np.array([0.5 for _ in range(len(clean_valid_X))])
print('0.5 loss: ' + str(log_loss(validation_labels, allpoint5_pred)))

print('Simple Averaging Loss: ' + str(log_loss(validation_labels, model_pred.clip(0.35, 0.65))))
print(
    'Two Times Larger Range(Averaging) Loss: ' + str(log_loss(validation_labels, larger_range(model_pred, 2).clip(0.35, 0.65))))
print('Best Single Model Loss: ' + str(log_loss(validation_labels, best_model_pred.clip(0.35, 0.65))))
print('Two Times Larger Range(Single Model) Loss: ' + str(
    log_loss(validation_labels, larger_range(best_model_pred, 2).clip(0.35, 0.65))))

if log_loss(validation_labels, model_pred.clip(0.35, 0.65)) < log_loss(validation_labels, larger_range(model_pred, 2).clip(0.35, 0.65)):
    two_times = False
...
check_answers(final_preds, final_labels)
correct_precentile(final_preds, final_labels)
#%% ROC Computation
# roc curve and auc
from sklearn.datasets import make_classification
from sklearn.linear_model import LogisticRegression
...
testy = y
lr_probs = probs
ns_probs = random_pred
# calculate scores
ns_auc = roc_auc_score(testy, ns_probs)
lr_auc = roc_auc_score(testy, lr_probs)
# summarize scores
print('No Skill: ROC AUC=%.3f' % (ns_auc))
print('CNN-LSTM: ROC AUC=%.3f' % (lr_auc))
# calculate roc curves
ns_fpr, ns_tpr, _ = roc_curve(testy, ns_probs)
lr_fpr, lr_tpr, _ = roc_curve(testy, lr_probs)
# plot the roc curve for the model
pyplot.plot(ns_fpr, ns_tpr, linestyle='--', label='Random Preds')
pyplot.plot(lr_fpr, lr_tpr, marker='.', label='CNN-LSTM')
# axis labels
pyplot.xlabel('False Positive Rate')
pyplot.ylabel('True Positive Rate')
# show the legend
